        ...
        return this.props.userData.map((user)=>{
            if(user.username.toLowerCase().includes(this.props.searchVal.toLowerCase()))
                return <Fade collapse duration={500} when={true}>
                    <div className="searchResult">@{this.state.imgURL[user.username]}{user.username}</div></Fade>
            else
                return <Fade collapse duration={500} when={false}><div className="searchResult">{user.username}</div></Fade>
            }
        )
    }
